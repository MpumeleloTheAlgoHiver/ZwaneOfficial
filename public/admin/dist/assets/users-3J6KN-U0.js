import{supabase as f}from"./supabaseClient-WTCtVqgB.js";/* empty css              *//* empty css               */import{i as E}from"./layout-CF2NSHyg.js";import{n as I,o as L,j as _,p as B}from"./dataService-BdJkK1bK.js";import{v as y,a as m,b as C}from"./utils-CZwHw4kl.js";import"https://esm.sh/@supabase/supabase-js@2";import"./theme-CeTh6-N5.js";function $(t,e={}){const{showActions:a=!0,className:n="",isLuhnValid:l=!0}=e;let s=t.full_name||t.first_name+" "+t.surname;s=s.replace("NOT_PROVIDED","").trim();const r=s.split(" ").map(o=>o[0]).join("").substring(0,2).toUpperCase(),i=l?"bg-emerald-50 text-emerald-700 border-emerald-100":"bg-red-50 text-red-700 border-red-100",d=l?"check_circle":"warning",c=l?"Verified ID":"ID Error";return`
    ...
    `}let g=[],v=[],h=null,u=null,p=1;const b=20;let T="all";const D=`
...
`,S=t=>["admin","super_admin","base_admin"].includes(t),M=t=>({super_admin:"SUPER ADMIN",admin:"BRANCH MANAGER",base_admin:"LOAN OFFICER"})[t]||"CLIENT",A=t=>{const e=(t||"UNKNOWN").toUpperCase();let a="bg-gray-100 text-gray-600";return e==="DISBURSED"&&(a="bg-green-100 text-green-700"),e==="DECLINED"&&(a="bg-red-100 text-red-700"),["STARTED","SUBMITTED"].includes(e)&&(a="bg-blue-50 text-blue-700"),`<span class="px-2 py-0.5 rounded text-[10px] font-bold ${a}">${e}</span>`};window.switchView=t=>{const e=document.getElementById("view-list"),a=document.getElementById("view-detail");t==="detail"?(e.classList.add("hidden"),a.classList.remove("hidden")):(e.classList.remove("hidden"),a.classList.add("hidden"),u=null)};window.openUserDetail=async t=>{try{document.body.style.cursor="wait";const e=await I(t);u=e;const a=e.profile,n=y(a.identity_number||a.id_number),l=document.getElementById("profile-card-container");l&&(l.innerHTML=$(a,{isLuhnValid:n}));const s=e.financials||{};document.getElementById("detail-income").textContent=m(s.monthly_income||0),document.getElementById("detail-expenses").textContent=m(s.monthly_expenses||0),document.getElementById("stat-total-loans").textContent=e.loans.length,document.getElementById("stat-total-docs").textContent=e.documents.length;const r=e.loans.filter(o=>["DISBURSED","ACTIVE"].includes(o.status)).reduce((o,k)=>o+Number(k.amount),0);document.getElementById("stat-active-debt").textContent=m(r);const i=document.getElementById("detail-loans-body");e.loans.length===0?i.innerHTML='<tr><td colspan="5" class="p-12 text-center text-xs font-bold text-slate-300">No applications found.</td></tr>':i.innerHTML=e.loans.map(o=>`
                <tr class="hover:bg-slate-50 transition-colors cursor-pointer group" onclick="window.location.href='/admin/application-detail?id=${o.id}'">
                    ...
                    <td class="px-6 py-5 text-xs font-bold text-slate-600">${C(o.created_at)}</td>
                    <td class="px-6 py-5 text-sm font-black text-slate-900">${m(o.amount)}</td>
                    <td class="px-6 py-5">${A(o.status)}</td>
            ...
            `).join("");const c=document.getElementById("btn-transfer-branch");c&&(c.onclick=()=>window.openBranchModal()),window.switchView("detail")}catch(e){console.error("Detail Error:",e),alert("Could not load user details.")}finally{document.body.style.cursor="default"}};window.openBranchModal=()=>{if(!u)return;const t=u.profile;document.getElementById("modal-username").textContent=t.full_name;const e=document.getElementById("modal-branch-select");e.innerHTML='<option value="online">Online / Unassigned</option>',v.forEach(a=>{const n=document.createElement("option");n.value=a.id,n.textContent=a.name,t.branch_id===a.id&&(n.selected=!0),e.appendChild(n)}),document.getElementById("branch-modal").classList.remove("hidden")};window.confirmBranchTransfer=async()=>{const t=document.querySelector("#branch-modal button.bg-orange-600"),e=document.getElementById("modal-branch-select").value,a=u.profile;try{t.disabled=!0,t.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Moving...';const n=e==="online"?null:e,{error:l}=await f.from("profiles").update({branch_id:n}).eq("id",a.id);if(l)throw l;await f.from("loan_applications").update({branch_id:n}).eq("user_id",a.id),alert("Success! User transferred."),document.getElementById("branch-modal").classList.add("hidden"),window.location.reload()}catch(n){alert("Transfer failed: "+n.message),t.disabled=!1,t.textContent="Confirm Transfer"}};const w=t=>{const e=document.getElementById("users-table-body");if(!e)return;const a=(p-1)*b,n=t.slice(a,a+b);if(n.length===0){e.innerHTML='<tr><td colspan="5" class="p-20 text-center text-slate-300 font-bold">No results matching your query.</td></tr>';return}e.innerHTML=n.map(s=>{const r=s.branches?.name||"Online",i=y(s.identity_number||s.id_number);return`
