import{supabase as k}from"./supabaseClient-WTCtVqgB.js";/* empty css              *//* empty css               */import{i as W,g as j}from"./layout-DLkpXMPI.js";import{f as y,a as F}from"./utils-CZwHw4kl.js";import{f as $,a as B,b as Y,c as H,d as U,e as K}from"./dataService-489HhSyD.js";import"https://esm.sh/@supabase/supabase-js@2";import"./theme-CeTh6-N5.js";let D=!1;async function q(){if(!D){D=!0;try{const{data:{session:e},error:r}=await k.auth.getSession();if(r||!e){console.log("🔒 Admin session invalid - redirecting to login"),window.location.replace("/auth/login.html");return}const n=["base_admin","admin","super_admin","owner"],a=(e.user?.app_metadata?.role||e.user?.user_metadata?.role||"").toLowerCase();let t=n.includes(a);if(!t){const{data:o,error:s}=await k.rpc("is_role_or_higher",{p_min_role:"base_admin"});!s&&o&&(t=!0)}if(!t){console.log("🔒 Not an admin - access denied. Role:",a),await k.auth.signOut(),window.location.replace("/auth/login.html");return}console.log("✅ Admin session validated")}catch(e){console.error("Admin session guard error:",e),window.location.replace("/auth/login.html")}finally{D=!1}}}q();const V={enabled:!0,easing:"easeinout",speed:900,animateGradually:{enabled:!0,delay:120},dynamicAnimation:{enabled:!0,speed:400}};function h(e){return e.chart||(e.chart={}),e.chart.animations={...V,...e.chart.animations||{}},e}const G=()=>new Promise((e,r)=>{if(window.ApexCharts)return e();const n=document.createElement("script");n.src="https://cdn.jsdelivr.net/npm/apexcharts",n.onload=e,n.onerror=r,document.head.appendChild(n)}),z="admin-dashboard-analytics-style",b=()=>{const e=getComputedStyle(document.documentElement),r=(e.getPropertyValue("--color-primary")||"#0ea5e9").trim()||"#0ea5e9",n=(e.getPropertyValue("--color-secondary")||"#f97316").trim()||"#f97316";return{primary:r,secondary:n}};async function X(){try{const e=await fetch("/api/suresystems/activation-status");if(!e.ok)throw new Error(`SureSystems status fetch failed (${e.status})`);return await e.json()}catch(e){return console.warn("SureSystems activation status unavailable:",e.message||e),null}}document.addEventListener("DOMContentLoaded",async()=>{try{await G()}catch(l){console.error("ApexCharts failed to load:",l);const d=document.getElementById("main-content");d&&(d.innerHTML='<div class="p-8 text-center text-red-600 font-semibold">Charts failed to load. Check your connection and refresh.</div>');return}if(!await W())return;const r=j(),n=document.getElementById("main-content");if(!document.getElementById(z)){const l=document.createElement("style");l.id=z,l.innerHTML=`
    ...
    `,document.head.appendChild(l)}let a={text:"Operational",color:"#10b981"},t,o,s,c,u,i,p=null;try{[t,o,s,c,u,i,p]=await Promise.all([$().catch(()=>({financials:{},portfolioStatus:[]})),B().catch(()=>({data:[]})),Y().catch(()=>({data:[]})),H().catch(()=>({data:{}})),U().catch(()=>({data:null})),K().catch(()=>({data:[]})),X()])}catch(l){console.error("System Fetch Error:",l),a={text:"System Error",color:"#ef4444",dot:"bg-red-500"}}const f=t?.financials||{},m=o?.data||[],L=s?.data||[],P=i?.data||[],I=c?.data||{},g=u?.data||ie(m,L);b();const x=(()=>{if(!p?.configured)return{text:"SureSystems: Not Configured",color:"#f59e0b",dot:"bg-amber-500"};const l=Number(p?.recent?.failed||0),d=Number(p?.recent?.success||0),T=p?.recent?.lastAttemptAt,M=T?(Date.now()-new Date(T).getTime())/(1e3*60*60*24):999;return l>0&&d===0&&M<=7?{text:"SureSystems: UAT Credentials Rejected (401) — Contact SureDebit",color:"#ef4444",dot:"bg-red-500"}:l>0&&d===0&&M>7?{text:"SureSystems: Connected",color:"#10b981",dot:"bg-emerald-500"}:{text:d>0?`SureSystems: ${d} mandates activated`:"SureSystems: Connected",color:"#10b981",dot:"bg-emerald-500"}})(),A=t?.financials?.pending_apps||0,O=new Date().toLocaleDateString("en-ZA",{weekday:"long",month:"long",day:"numeric",year:"numeric"});n.innerHTML=`
