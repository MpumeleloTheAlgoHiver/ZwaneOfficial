...
<div style="max-width:780px;margin:0 auto">
  ...
  `,e.classList.remove("hidden"),setTimeout(()=>{e.classList.add("hidden")},5e3)},_e=async()=>{const n=document.getElementById("contract-status-empty"),t=document.getElementById("contract-status-section");if(!W()){j(),t&&t.classList.add("hidden"),n&&(n.classList.remove("hidden"),n.innerHTML=`
      ...
      `);return}n&&(n.classList.remove("hidden"),n.textContent="No contracts sent yet."),await T()},Ee=async(n=null)=>{if(!l||!l.profiles){alert("Error: Application data not loaded");return}const t=n||document.getElementById("btn-send-contract"),e=t?t.innerHTML:"";t&&(t.disabled=!0,t.innerHTML='<span class="material-symbols-outlined text-[16px] animate-spin align-middle">progress_activity</span> Sending...');try{const a=await re(l,l.profiles);alert(`✅ Contract sent successfully to ${l.profiles.email}`),await $(l.id,"OFFERED"),await T(),await E()}catch(a){console.error("Send contract error:",a),alert(`❌ Failed to send contract: ${a.message}`)}finally{t&&(t.disabled=!1,t.innerHTML=e)}},Se=()=>{l?.id&&window.open(`/api/contracts/${l.id}/preview`,"_blank")},ke=()=>{if(!l)return!1;const n=l.status||"";return["OFFERED"].includes(n)},Ce=()=>{D||!ke()||(D=setInterval(()=>{T(!0)},ve))},j=()=>{D&&(clearInterval(D),D=null)},$e=async()=>{if(!(M||L||!l)){M=!0,L=!0,j();try{let n=!1;if(l.status!=="OFFER_ACCEPTED"){const{error:t}=await $(l.id,"OFFER_ACCEPTED");if(t){console.error("Auto advance to Contract Signed failed:",t),L=!1;return}l.status="OFFER_ACCEPTED",l.contract_signed_at=new Date().toISOString(),n=!0}if(n)try{const t=await fetch("/api/suresystems/activate-application",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({applicationId:l.id})}),e=await t.json().catch(()=>({}));if(!t.ok||e?.success===!1){const a=new Error(e?.error||e?.message||"SureSystems mandate activation failed");throw a.details=e?.details||null,a}}catch(t){console.error("SureSystems activation failed during contract auto-complete:",{message:t?.message||"Unknown activation error",details:t?.details||null}),u(t?.message||"SureSystems mandate activation failed","error")}F(l),P("OFFER_ACCEPTED"),u("Contract signed! Advanced to approval phase.","success"),await E()}catch(n){console.error("handleContractCompleted error:",n),L=!1}finally{M=!1}}},T=async(n=!1)=>{if(l?.id)try{const t=await le(l.id),e=document.getElementById("contract-status-section"),a=document.getElementById("contract-status-empty");if(t.length===0){e&&e.classList.add("hidden"),a&&(a.classList.remove("hidden"),a.textContent="No contracts sent yet."),j(),K(!1);return}a&&a.classList.add("hidden"),e&&e.classList.remove("hidden"),Ie(t);const s=t[0]?.status?.toLowerCase?.()||"";K(s==="declined"),s==="completed"&&!L?await $e():s!=="completed"&&!n&&Ce()}catch(t){console.error("Load contract status error:",t)}},Ie=n=>{const t=document.getElementById("contract-status-content");t&&(t.innerHTML=n.map(e=>{const a=Ae(e.status),s=Le(e.status);return`
...
${a?.message||"Unknown activation error"}`+s),u(a?.message||"SureSystems mandate activation failed","error")}}else u("Status manually updated.","success");await E()}}};const C=document.getElementById("confirmation-modal"),z=document.getElementById("modal-title"),G=document.getElementById("modal-body"),Be=(n,t,e)=>{z&&(z.textContent=n),G&&(G.textContent=t),N=e,C?(C.classList.remove("hidden"),C.classList.add("flex")):confirm(t)&&e()},O=()=>{C&&(C.classList.add("hidden"),C.classList.remove("flex")),N=null};window.closeSACRRAGate=()=>{document.getElementById("sacrra-gate-modal")?.classList.add("hidden")};window.approveAnywaySkipSACRRA=async()=>{window.closeSACRRAGate(),u("Approving loan — remember to complete SACRRA fields before monthly submission.","success"),await b.from("sacrra_rejections").upsert([{match_key:`PENDING-${l?.id}`,error_code:"W00",error_message:`Application ${l?.loan_number||l?.id} approved with incomplete SACRRA fields. Fix before monthly submission.`,resolved:!1}],{onConflict:"match_key,error_code",ignoreDuplicates:!0}).catch(()=>{}),H()};window.saveSACRRAAndApprove=async()=>{const n=document.getElementById("sacrra-gate-save-btn");n.disabled=!0,n.innerHTML='<span class="material-symbols-outlined text-[18px] animate-spin">progress_activity</span> Saving…';try{const t={},e={};if(document.querySelectorAll("#sacrra-gate-fields [data-source][data-field]").forEach(a=>{const s=a.dataset.field,r=a.dataset.source,o=a.value?.trim();o&&(r==="profile"&&(t[s]=o),r==="application"&&(e[s]=o))}),Object.keys(t).length){if(t.first_name||t.last_name){const s=t.first_name||l.profiles?.first_name||"",r=t.last_name||l.profiles?.last_name||l.profiles?.full_name?.split(" ").pop()||"";t.full_name=`${s} ${r}`.trim()}const{error:a}=await b.from("profiles").update({...t,updated_at:new Date().toISOString()}).eq("id",l.user_id);if(a)throw new Error("Profile save failed: "+a.message)}if(Object.keys(e).length){const{error:a}=await b.from("loan_applications").update({...e,updated_at:new Date().toISOString()}).eq("id",l.id);if(a)throw new Error("Application save failed: "+a.message)}window.closeSACRRAGate(),u("SACRRA fields saved — proceeding with approval…","success"),await E(),setTimeout(()=>H(),600)}catch(t){u("Save failed: "+t.message,"error"),n.disabled=!1,n.innerHTML='<span class="material-symbols-outlined text-[18px]">save</span> Save & Approve Loan'}};const H=async()=>{const{data:{user:n}}=await b.auth.getUser(),{data:t}=await Z(l.id);if(t&&t.length>0){u("Disbursement already exists for this application.","error");return}const{data:e,error:a}=await $(l.id,"APPROVED");if(a){u(a.message,"error");return}const{data:s}=await J(),r=l.bank_account?.id||null,{data:o,error:i}=await ne({applicationId:l.id,userId:l.user_id,amount:e.amount,bankAccountId:r,createdBy:n.id});if(i)u("Status updated but disbursement creation failed: "+i.message,"error");else{let c="Application approved & disbursement created.";o?.payout_method==="cashsend"&&o?.cashsend_fee&&(c+=` CashSend fee: R${o.cashsend_fee.toFixed(2)}`),u(c,"success")}E()},Te=async()=>{O();const n=l?.profiles||{},e=ye(l||{},n);if(!e.passed){const a=document.getElementById("sacrra-gate-fields"),s=e.issues.filter(i=>i.source==="profile"),r=e.issues.filter(i=>i.source==="application"),o=(i,c)=>c.length===0?"":`
      <div class="mb-6">
      ...
      <div class="mb-4 flex items-center gap-2 p-3 bg-amber-50 rounded-xl border border-amber-200">
        ...
        <div>
          <p class="text-sm font-bold text-amber-800">${e.issues.length} field${e.issues.length>1?"s":""} needed for the monthly SACRRA submission.</p>
  ...
  `,window._incomeData={salary:c,otherIncome:d,totalExpenses:g};const x=t&&t.length>0?t[0]:null;if(x){const m=x.credit_score||0,f=m>600?"text-green-600":m>500?"text-yellow-600":"text-red-600";if(r&&(r.textContent=`Checked on ${_(x.checked_at||x.created_at||new Date)}`),o){const w=x.raw_xml_data;w?(o.classList.remove("hidden"),o.innerHTML='<span class="material-symbols-outlined text-[16px] mr-1">picture_as_pdf</span> View Bureau Report',o.className="text-sm text-white px-4 py-2 rounded-xl transition-colors shadow-sm font-semibold flex items-center gap-1",o.style.background="var(--color-primary)",o.onclick=()=>window.viewBureauReport(w)):o.classList.add("hidden")}s.innerHTML=`
        ...
        <div class="space-y-2">
          ${h.map(f=>`
            <div class="flex items-start gap-3 p-3 bg-white rounded-xl border border-red-100">
              ...
              <div>
                <p class="text-sm font-semibold text-red-900">${f.label||f.rule_key||"Rule Failed"}</p>
                ${f.reason?`<p class="text-xs text-red-600 mt-0.5">${f.reason}</p>`:""}
              </div>
            </div>`).join("")}
    ...
    `,a.appendChild(d)}e&&[{key:"id_front",label:"KYC ID Front",url:e.id_front_image_url},{key:"id_back",label:"KYC ID Back",url:e.id_back_image_url},{key:"selfie",label:"KYC Selfie",url:e.selfie_image_url}].filter(c=>c.url).forEach(c=>{const d=document.createElement("div");d.className="flex items-center justify-between p-4 bg-purple-50/50 border border-purple-200 rounded-xl hover:border-purple-400 transition-all mt-4",d.innerHTML=`
      ...
      `,a.appendChild(d)}),Me()},Me=()=>{document.querySelectorAll(".admin-doc-upload").forEach(n=>{n.addEventListener("change",async t=>{const e=t.target.files[0];if(!e||!l)return;const a=t.target.dataset.type,s=t.target.parentElement,r=s.childNodes[0].textContent;s.childNodes[0].textContent="Processing...";try{const{data:{session:o}}=await b.auth.getSession(),i=o.user.id,c=e.name.split(".").pop(),d=`${a}_${Date.now()}.${c}`,p=`${i}/${l.user_id}_${d}`,{error:g}=await b.storage.from("client_docs").upload(p,e,{upsert:!0});if(g)throw g;const{error:v}=await b.rpc("register_admin_upload",{p_user_id:l.user_id,p_app_id:l.id,p_file_name:d,p_original_name:e.name,p_file_path:p,p_file_type:a,p_mime_type:e.type,p_file_size:e.size});if(v)throw v;u("Document Updated Successfully","success"),E()}catch(o){console.error(o),u(o.message,"error")}finally{s.childNodes[0].textContent=r}})})};window.handleSmartDownload=async n=>{try{let t=n;n.includes("/storage/v1/object/")&&(t=n.split("/").slice(8).join("/"));let{data:e,error:a}=await b.storage.from("client_docs").createSignedUrl(t,60);if((a||!e)&&({data:e,error:a}=await b.storage.from("documents").createSignedUrl(t,60)),a)throw a;window.open(e.signedUrl,"_blank")}catch(t){console.error("Smart Download Error:",t),u("File not found in any bucket. Please check storage manually.","error")}};const Pe=async(n,t,e)=>{const a=document.getElementById("loan-history-list"),s=document.getElementById("app-history-list");let r=document.getElementById("admin-metadata-container");if(e){const o=document.getElementById("loan-tab");if(!r){r=document.createElement("div"),r.id="admin-metadata-container",r.className="mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-gray-100 pt-8";const i=Array.from(o.querySelectorAll("h3")).find(c=>c.textContent.includes("Client History"));i?o.insertBefore(r,i):o.appendChild(r)}try{const i=[e.created_by_admin,e.reviewed_by_admin].filter(Boolean),{data:c}=await b.from("profiles").select("id, full_name").in("id",i),d=c?.find(g=>g.id===e.created_by_admin)?.full_name||"System / User",p=c?.find(g=>g.id===e.reviewed_by_admin)?.full_name||"Pending Review";r.innerHTML=`
  ...
  `;const h=o.signature_data,m=o.credit_life_signature_data,f=(w,I,S)=>`
  ...
  `,t.classList.remove("hidden"),t.classList.add("flex")},Y=()=>{const n=document.getElementById("credit-life-contract-modal");n&&(n.classList.add("hidden"),n.classList.remove("flex"))},Ve=async n=>{const t=document.getElementById("action-buttons-container");if(t)try{const{data:e}=await Z(n.id),{data:a}=await J();if(!e||e.length===0){t.innerHTML=`
  ...
  `,e&&(e.textContent=t.replace("_"," "),e.className=`mt-2 text-lg font-bold uppercase tracking-wide ${U(t).split(" ")[0].replace("bg-","text-").replace("-100","-600")}`);const f=document.getElementById("status-override-select"),w=document.getElementById("manual-update-btn"),I=document.getElementById("override-hint");if(t==="DISBURSED"?(f&&(f.disabled=!0),w&&(w.disabled=!0,w.classList.add("opacity-50","cursor-not-allowed"),w.innerText="Locked"),I&&(I.textContent="🔒 Application is active. Modifications disabled.")):(f&&(f.disabled=!1,f.value=t),w&&(w.disabled=!1,w.innerText="Update")),a&&(a.className="mt-3 p-3 rounded-lg text-xs font-medium leading-relaxed hidden",t==="OFFERED"?(a.textContent="Contract Sent. Waiting for user to sign.",a.classList.add("bg-purple-50","text-purple-700","block")):t==="APPROVED"&&(a.textContent="Application is queued for disbursement.",a.classList.add("bg-green-50","text-green-700","block"))),s){if(s.innerHTML="",["BUREAU_OK","BANK_LINKING","STARTED","AFFORD_REFER","BUREAU_REFER"].includes(t)){const S=t==="AFFORD_REFER"||t==="BUREAU_REFER"?'<div class="p-3 bg-orange-50 border border-orange-100 rounded-lg mb-3 text-xs text-orange-700 font-bold"><i class="fa-solid fa-circle-exclamation mr-1"></i> Currently Under Manual Review</div>':"";s.innerHTML=`
          ...
          `,document.getElementById("btn-approve-contract").onclick=()=>Be("Approve","Mark contract as valid and ready for payout?",Te);else if(t==="APPROVED")Ve(l);else if(t==="DISBURSED")s.innerHTML=`
