import{supabase as f}from"./supabaseClient-WTCtVqgB.js";/* empty css              *//* empty css               */import{i as k}from"./layout-DLkpXMPI.js";import{n as E,o as I,j as L,p as _}from"./dataService-BdJkK1bK.js";import{v,a as m,b as B}from"./utils-CZwHw4kl.js";import"https://esm.sh/@supabase/supabase-js@2";import"./theme-CeTh6-N5.js";function $(t,e={}){const{showActions:a=!0,className:n="",isLuhnValid:l=!0}=e;let s=t.full_name||t.first_name+" "+t.surname;s=s.replace("NOT_PROVIDED","").trim();const d=s.split(" ").map(o=>o[0]).join("").substring(0,2).toUpperCase(),i=l?"bg-emerald-50 text-emerald-700 border-emerald-100":"bg-red-50 text-red-700 border-red-100",r=l?"check_circle":"warning",c=l?"Verified ID":"ID Error";return`
...
`,U=t=>["admin","super_admin","base_admin"].includes(t),S=t=>({super_admin:"SUPER ADMIN",admin:"BRANCH MANAGER",base_admin:"LOAN OFFICER"})[t]||"CLIENT",M=t=>{const e=(t||"UNKNOWN").toUpperCase();let a="bg-gray-100 text-gray-600";return e==="DISBURSED"&&(a="bg-green-100 text-green-700"),e==="DECLINED"&&(a="bg-red-100 text-red-700"),["STARTED","SUBMITTED"].includes(e)&&(a="bg-blue-50 text-blue-700"),`<span class="px-2 py-0.5 rounded text-[10px] font-bold ${a}">${e}</span>`};window.switchView=t=>{const e=document.getElementById("view-list"),a=document.getElementById("view-detail");t==="detail"?(e.classList.add("hidden"),a.classList.remove("hidden")):(e.classList.remove("hidden"),a.classList.add("hidden"),u=null)};window.openUserDetail=async t=>{try{document.body.style.cursor="wait";const e=await E(t);u=e;const a=e.profile,n=v(a.identity_number||a.id_number),l=document.getElementById("profile-card-container");l&&(l.innerHTML=$(a,{isLuhnValid:n}));const s=e.financials||{};document.getElementById("detail-income").textContent=m(s.monthly_income||0),document.getElementById("detail-expenses").textContent=m(s.monthly_expenses||0),document.getElementById("stat-total-loans").textContent=e.loans.length,document.getElementById("stat-total-docs").textContent=e.documents.length;const d=e.loans.filter(o=>["DISBURSED","ACTIVE"].includes(o.status)).reduce((o,w)=>o+Number(w.amount),0);document.getElementById("stat-active-debt").textContent=m(d);const i=document.getElementById("detail-loans-body");e.loans.length===0?i.innerHTML='<tr><td colspan="5" class="p-12 text-center text-xs font-bold text-slate-300">No applications found.</td></tr>':i.innerHTML=e.loans.map(o=>`
            ...
            `).join("");const r=document.getElementById("detail-docs-grid");e.documents.length===0?r.innerHTML='<div class="col-span-3 text-center text-[10px] font-black text-slate-400 py-8 border-2 border-dashed border-slate-50 rounded-3xl">No documents found</div>':r.innerHTML=e.documents.map(o=>`
