import{supabase as b}from"./supabaseClient-WTCtVqgB.js";/* empty css              *//* empty css               */import{i as Q}from"./layout-BuEx1KZr.js";import{b as g,a as f}from"./utils-CZwHw4kl.js";import{t as $,v as W,w as X,x as ee}from"./dataService-P9YE594z.js";import"https://esm.sh/@supabase/supabase-js@2";import"./theme-BmY-8nnP.js";const R="/api/docuseal",te=void 0;function Z(){return!!te}async function ne(t,n){try{if(!Z())throw new Error("DocuSeal integration is disabled");const e=await fetch(`${R}/send-contract`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({applicationData:t,profileData:n})});if(!e.ok){const r=await e.json().catch(()=>({}));throw console.error("DocuSeal proxy error:",e.status,r),new Error(r.error||r.message||`Failed to send contract: ${e.status}`)}const a=await e.json();if(!a||!Array.isArray(a)||a.length===0)throw new Error("Invalid response from DocuSeal API");const s=a[0];return await oe(s,t.id),{submission_id:s.submission_id,submitter_id:s.id,slug:s.slug,status:s.status,embed_src:s.embed_src,email:s.email}}catch(e){throw console.error("DocuSeal send contract error:",e),e}}async function ae(t){try{const n=await fetch(`${R}/submissions/${t}`);if(!n.ok){const e=await n.json().catch(()=>({}));throw new Error(e.error||e.message||`Failed to fetch submission status: ${n.status}`)}return await n.json()}catch(n){throw console.error("DocuSeal get status error:",n),n}}async function se(t){try{const n=await fetch(`${R}/submitters/${t}`);if(!n.ok){const e=await n.json().catch(()=>({}));throw new Error(e.error||e.message||`Failed to fetch submitter details: ${n.status}`)}return await n.json()}catch(n){throw console.error("DocuSeal get submitter error:",n),n}}async function re(t){try{const{data:n,error:e}=await b.from("docuseal_submissions").select("*").eq("application_id",t).order("created_at",{ascending:!1});if(e)throw e;return n||[]}catch(n){return console.error("Error fetching submissions:",n),[]}}async function oe(t,n){try{const{error:e}=await b.from("docuseal_submissions").insert({application_id:n,submission_id:t.submission_id,submitter_id:t.id,slug:t.slug,status:t.status||"pending",email:t.email,name:t.name,role:t.role,embed_src:t.embed_src,sent_at:t.sent_at,opened_at:t.opened_at,completed_at:t.completed_at,metadata:t.metadata||{},created_at:new Date().toISOString()});if(e)throw e}catch(e){throw console.error("Error saving submission to database:",e),e}}async function ie(t,n,e={}){try{const{error:a}=await b.from("docuseal_submissions").update({status:n,...e,updated_at:new Date().toISOString()}).eq("submission_id",t);if(a)throw a}catch(a){throw console.error("Error updating submission status:",a),a}}async function le(t,n,e={}){try{const{error:a}=await b.from("docuseal_submissions").update({status:n,...e,updated_at:new Date().toISOString()}).eq("submitter_id",t);if(a)throw a}catch(a){throw console.error("Error updating submitter status:",a),a}}function H(t){return`https://docuseal.co/s/${t}`}async function de(t,n={}){try{const e=await fetch(`${R}/submitters/${t}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({send_email:!0,...n})});if(!e.ok){const s=await e.json().catch(()=>({}));throw new Error(s.error||s.message||`Failed to resend contract: ${e.status}`)}const a=await e.json();return await le(t,a.status,{sent_at:a.sent_at}),a}catch(e){throw console.error("DocuSeal resend error:",e),e}}async function ce(t){try{const n=await fetch(`${R}/submissions/${t}`,{method:"DELETE"});if(!n.ok){const a=await n.json().catch(()=>({}));throw new Error(a.error||a.message||`Failed to archive submission: ${n.status}`)}const e=await n.json();return await ie(t,"archived",{archived_at:e.archived_at}),e}catch(n){throw console.error("DocuSeal archive error:",n),n}}async function ue(t,n=null){try{const e=await ae(t);if(!e.submitters||e.submitters.length===0)throw new Error("No submitters found for this submission");if(n){const a=e.submitters.find(s=>s.email===n);if(!a)throw new Error(`No submitter found with email: ${n}`);return a.id}return e.submitters[0].id}catch(e){throw console.error("Error getting submitter ID:",e),e}}let l=null,F=null,V=!1,I=null,A=null,B=!1,O=!1;const pe=5e3,me=[{value:"STARTED",label:"Step 1: New Application"},{value:"BANK_LINKING",label:"Bank Analysis"},{value:"AFFORD_OK",label:"Step 3: Affordability OK"},{value:"AFFORD_REFER",label:"Affordability Refer"},{value:"OFFERED",label:"Step 4: Contract Sent"},{value:"OFFER_ACCEPTED",label:"Contract Signed"},{value:"READY_TO_DISBURSE",label:"Step 6: Approved — Queue Disburse"},{value:"DECLINED",label:"Declined"}],be=`
          ...
          <div class="p-6 bg-surface-container-lowest border-t border-outline-variant/10">
              ...
              <div class="flex gap-2">
                  <select id="status-override-select" class="flex-1 text-xs border-outline-variant/30 rounded-xl">
                      ${me.map(t=>`<option value="${t.value}">${t.label}</option>`).join("")}
                  </select>
...
<div style="max-width:780px;margin:0 auto">
...
</body></html>`),d.document.close()};const u=(t,n="success")=>{const e=document.getElementById("feedback-container");if(!e)return;const a=n==="success";e.innerHTML=`
  ...
  `,e.classList.remove("hidden"),setTimeout(()=>{e.classList.add("hidden")},5e3)},fe=async()=>{const t=document.getElementById("contract-status-empty"),n=document.getElementById("contract-status-section");if(!Z()){M(),n&&n.classList.add("hidden"),t&&(t.classList.remove("hidden"),t.innerHTML=`
      ...
      `);return}t&&(t.classList.remove("hidden"),t.textContent="No contracts sent yet."),await N()},xe=async(t=null)=>{if(!l||!l.profiles){alert("Error: Application data not loaded");return}const n=t||document.getElementById("btn-send-contract"),e=n?n.innerHTML:"";n&&(n.disabled=!0,n.innerHTML='<span class="material-symbols-outlined text-[16px] animate-spin align-middle">progress_activity</span> Sending...');try{const a=await ne(l,l.profiles);alert(`✅ Contract sent successfully to ${l.profiles.email}`),await $(l.id,"OFFERED"),await N(),await h()}catch(a){console.error("Send contract error:",a),alert(`❌ Failed to send contract: ${a.message}`)}finally{n&&(n.disabled=!1,n.innerHTML=e)}},ge=()=>{l?.id&&window.open(`/api/contracts/${l.id}/preview`,"_blank")},ye=()=>{if(!l)return!1;const t=l.status||"";return["OFFERED"].includes(t)},ve=()=>{A||!ye()||(A=setInterval(()=>{N(!0)},pe))},M=()=>{A&&(clearInterval(A),A=null)},he=async()=>{if(!(O||B||!l)){O=!0,B=!0,M();try{let t=!1;if(l.status!=="OFFER_ACCEPTED"){const{error:n}=await $(l.id,"OFFER_ACCEPTED");if(n){console.error("Auto advance to Contract Signed failed:",n),B=!1;return}l.status="OFFER_ACCEPTED",l.contract_signed_at=new Date().toISOString(),t=!0}if(t)try{const n=await fetch("/api/suresystems/activate-application",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({applicationId:l.id})}),e=await n.json().catch(()=>({}));if(!n.ok||e?.success===!1){const a=new Error(e?.error||e?.message||"SureSystems mandate activation failed");throw a.details=e?.details||null,a}}catch(n){console.error("SureSystems activation failed during contract auto-complete:",{message:n?.message||"Unknown activation error",details:n?.details||null}),u(n?.message||"SureSystems mandate activation failed","error")}j(l),P("OFFER_ACCEPTED"),u("Contract signed! Advanced to approval phase.","success"),await h()}catch(t){console.error("handleContractCompleted error:",t),B=!1}finally{O=!1}}},N=async(t=!1)=>{if(l?.id)try{const n=await re(l.id),e=document.getElementById("contract-status-section"),a=document.getElementById("contract-status-empty");if(n.length===0){e&&e.classList.add("hidden"),a&&(a.classList.remove("hidden"),a.textContent="No contracts sent yet."),M(),q(!1);return}a&&a.classList.add("hidden"),e&&e.classList.remove("hidden"),we(n);const s=n[0]?.status?.toLowerCase?.()||"";q(s==="declined"),s==="completed"&&!B?await he():s!=="completed"&&!t&&ve()}catch(n){console.error("Load contract status error:",n)}},we=t=>{const n=document.getElementById("contract-status-content");n&&(n.innerHTML=t.map(e=>{const a=_e(e.status),s=Ee(e.status);return`
    ...
    `}).join(""))},q=t=>{if(typeof t!="boolean"||!l||t===V)return;V=t;const n="contract-declined-banner",e=document.getElementById(n),a=document.getElementById("contract-status-card");if(t){if(!I&&l.status!=="DECLINED"&&(I=l.status),l.status="DECLINED",P("DECLINED"),j(l),!e&&a){const s=document.createElement("div");s.id=n,s.className="mt-3 p-3 bg-red-50 border border-red-100 rounded-xl text-xs text-red-700 font-semibold flex items-center gap-2",s.innerHTML=`
      ...
      `;const r=a.querySelector("h3");r&&r.parentNode?r.parentNode.insertBefore(s,r.nextSibling):a.prepend(s)}}else e&&e.remove(),I&&(l.status=I),I=null,j(l),P(l.status)},_e=t=>{const n=(t||"").toLowerCase(),e={pending:{bg:"bg-yellow-100",text:"text-yellow-600",badge:"bg-yellow-100 text-yellow-700"},completed:{bg:"bg-green-100",text:"text-green-600",badge:"bg-green-100 text-green-700"},expired:{bg:"bg-red-100",text:"text-red-600",badge:"bg-red-100 text-red-700"},voided:{bg:"bg-gray-100",text:"text-gray-600",badge:"bg-gray-100 text-gray-700"},declined:{bg:"bg-red-100",text:"text-red-600",badge:"bg-red-100 text-red-700"}};return e[n]||e.pending},Ee=t=>{const n=(t||"").toLowerCase(),e={pending:"schedule",completed:"check_circle",expired:"error",voided:"block",declined:"cancel"};return e[n]||e.pending};window.viewSubmission=async(t,n,e)=>{const a=r=>r&&r!=="undefined"&&r!=="null"?r:null,s=window.open("","_blank");try{let r=null;if(a(n))try{const o=await se(n),i=o?.slug||o?.submitter?.slug,d=o?.embed_src||o?.submitter?.embed_src;r=a(d)||(a(i)?H(i):null)}catch(o){console.warn("Live submitter lookup failed, falling back to stored values:",o)}if(r||(r=a(e)),!r&&a(t)&&(r=H(t)),!r){s&&s.close(),alert("Unable to open this contract — the signing link is missing. Try resending the contract.");return}s?s.location.href=r:window.open(r,"_blank")}catch(r){console.error("viewSubmission error:",r),s&&s.close(),alert(`Could not open contract: ${r.message||r}`)}};window.resendSubmission=async(t,n=null)=>{if(confirm("Resend contract email to the applicant?"))try{let e=t;if(!e){if(!n)throw new Error("Unable to determine DocuSeal submitter");e=await ue(n)}await de(e),alert("✅ Contract email resent successfully"),await N()}catch(e){alert(`❌ Failed to resend: ${e.message}`)}};window.voidSubmission=async t=>{if(confirm("Void this contract submission? This cannot be undone."))try{await ce(t),alert("✅ Submission voided successfully"),await N()}catch(n){alert(`❌ Failed to void: ${n.message}`)}};const Se=()=>{const t=document.querySelectorAll(".tab-btn"),n=document.querySelectorAll(".tab-pane");t.forEach(e=>{e.addEventListener("click",()=>{t.forEach(r=>{r.classList.remove("active"),r.style.borderColor="",r.style.color="",r.classList.add("text-outline","border-transparent")}),e.classList.remove("text-outline","border-transparent"),e.classList.add("active"),e.style.borderColor="var(--color-primary)",e.style.color="var(--color-primary)",n.forEach(r=>r.classList.add("hidden"));const a=e.getAttribute("data-tab")+"-tab",s=document.getElementById(a);if(s&&s.classList.remove("hidden"),e.getAttribute("data-tab")==="audit"){const r=new URLSearchParams(window.location.search);J(r.get("id"))}})})};window.updateStatus=async t=>{if(t==="AFFORD_OK"){if(!(l.bureau_score_band||["BUREAU_OK","BANK_LINKING"].includes(l.status))){u("Cannot confirm affordability — no bureau result on record. Run the credit check first.","error");return}const{data:a}=await b.from("financial_profiles").select("monthly_income").eq("user_id",l.user_id).maybeSingle();if(!a?.monthly_income){u("Cannot confirm affordability — no income on record. Complete open banking first.","error");return}}if(t==="OFFERED"&&(!l.offer_principal||l.offer_principal<=0)){u("Cannot send contract — loan offer not configured yet.","error");return}if(t==="READY_TO_DISBURSE"){if(l.status!=="OFFER_ACCEPTED"&&!l.contract_signed_at){u("Cannot queue for disbursement — contract has not been signed yet.","error");return}if(!l.bank_account_id){u("Cannot queue for disbursement — no bank account linked.","error");return}}const{error:n}=await $(l.id,t);n?u(n.message,"error"):(u(`Status updated to ${t}`,"success"),h()),C()};window.declineApplication=async()=>{const{error:t}=await $(l.id,"DECLINED");t?u(t.message,"error"):(u("Application declined.","success"),h()),C()};window.saveNotes=async()=>{const t=document.getElementById("detail-notes").value,n=document.getElementById("btn-save-notes");if(!t.trim())return;const e=n.innerHTML;n.disabled=!0,n.innerHTML='<span class="material-symbols-outlined text-[14px] align-middle animate-spin mr-1">progress_activity</span> Saving...';try{const{error:a}=await W(l.id,t);if(a)throw a;u("Notes saved successfully","success"),n.innerHTML='<span class="material-symbols-outlined text-[14px] align-middle mr-1">check</span> Saved!',n.style.background="#16a34a",setTimeout(()=>{n.innerHTML=e,n.disabled=!1,n.style.background="var(--color-primary)"},2e3)}catch(a){u(a.message,"error"),n.disabled=!1,n.innerHTML=e}};window.saveRepaymentDate=async()=>{const t=document.getElementById("new-repayment-date");if(!t||!t.value)return;const n=t.value,e=document.getElementById("btn-save-date"),a=e.innerHTML;e.disabled=!0,e.innerHTML='<span class="material-symbols-outlined text-[14px] animate-spin">progress_activity</span>';try{const r={...l.offer_details||{},first_payment_date:n},{error:o}=await b.from("loan_applications").update({offer_details:r,repayment_start_date:n}).eq("id",l.id);if(o)throw o;u("First repayment date updated successfully","success"),await h()}catch(s){console.error("Date Update Error:",s),u(s.message,"error"),e.disabled=!1,e.innerHTML=a}};const ke=t=>{const n=document.getElementById("contract-date-label"),e=document.getElementById("contract-date-status-badge"),a=document.getElementById("contract-set-date-btn"),s=document.getElementById("new-repayment-date"),r=document.getElementById("contract-date-view"),o=document.getElementById("contract-date-edit");if(!n||!e||!a||!s||!r||!o||!t)return;const i=t.repayment_start_date||t.offer_details?.first_payment_date,d=new Date;d.setHours(0,0,0,0),n.textContent=i?g(i):"Not Scheduled",e.textContent=i?"Date set":"Not set",e.className=i?"px-2 py-0.5 text-[10px] font-bold rounded-full bg-green-100 text-green-700":"px-2 py-0.5 text-[10px] font-bold rounded-full bg-yellow-100 text-yellow-700",s.value=i?new Date(i).toISOString().split("T")[0]:"",s.min=d.toISOString().split("T")[0];const c=t.status==="DISBURSED";a.disabled=c,a.classList.toggle("opacity-50",c),a.classList.toggle("cursor-not-allowed",c),r.classList.remove("hidden"),o.classList.add("hidden")};window.toggleContractDateEdit=()=>{const t=document.getElementById("contract-date-view"),n=document.getElementById("contract-date-edit");t&&n&&(t.classList.toggle("hidden"),n.classList.toggle("hidden"))};window.manualStatusChange=async()=>{if(l.status==="DISBURSED"){alert(`⛔ ACTION BLOCKED

This application has already been disbursed. To maintain financial integrity, you cannot change the status of an active loan.`);return}const n=document.getElementById("status-override-select").value;if(n!==l.status){if(n.includes("BUREAU")){alert("Cannot manually override Bureau statuses. These are automated.");return}if(confirm(`Are you sure you want to manually force status to "${n}"?`)){const{error:e}=await $(l.id,n);if(e){u(e.message,"error");return}if(n==="OFFER_ACCEPTED"){u("Status manually updated. Activating SureSystems mandate...","success");try{const a=await fetch("/api/suresystems/activate-application",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({applicationId:l.id})}),s=await a.json().catch(()=>({}));if(!a.ok||s?.success===!1){const r=new Error(s?.error||s?.message||"SureSystems mandate activation failed");throw r.details=s?.details||null,r}alert(`✅ SureSystems mandate activated successfully.
...
${a?.message||"Unknown activation error"}`+s),u(a?.message||"SureSystems mandate activation failed","error")}}else u("Status manually updated.","success");await h()}}};const k=document.getElementById("confirmation-modal"),K=document.getElementById("modal-title"),z=document.getElementById("modal-body"),Ce=(t,n,e)=>{K&&(K.textContent=t),z&&(z.textContent=n),F=e,k?(k.classList.remove("hidden"),k.classList.add("flex")):confirm(n)&&e()},C=()=>{k&&(k.classList.add("hidden"),k.classList.remove("flex")),F=null},$e=async()=>{const{data:{user:t}}=await b.auth.getUser(),{data:n}=await b.from("payouts").select("id").eq("application_id",l.id).maybeSingle();if(n){u("A payout record already exists for this application.","error"),C();return}const{data:e,error:a}=await $(l.id,"READY_TO_DISBURSE");if(a){u(a.message,"error"),C();return}const s={application_id:l.id,user_id:l.user_id,amount:e.amount,status:"pending_disbursement"},{error:r}=await ee(s);r?u("Status updated but payout creation failed: "+r.message,"error"):(u("Application approved & financial values locked.","success"),h()),C()},Le=(t,n)=>{const e=t?.full_name||"Unknown User",a=t?.avatar_url||`https://ui-avatars.com/api/?name=${e.replace(" ","+")}&background=random`;document.getElementById("profile-image").src=a,document.getElementById("detail-fullname").textContent=e,document.getElementById("detail-email").textContent=t?.email||"N/A",document.getElementById("detail-mobile").textContent=t?.contact_number||t?.cell_tel_no||"N/A",je(t),Me(t);const s=document.getElementById("detail-nok");if(s){const i=[t?.nok_name,t?.nok_relationship,t?.nok_phone].filter(Boolean).join(" · ");s.textContent=i||"— Not provided —",s.style.color=i?"":"#ef4444"}const r=document.getElementById("detail-identity-number");r&&(r.textContent=t?.identity_number||"N/A");const o=document.getElementById("bank-accounts-container");o&&(o.innerHTML="",n&&n.length>0?n.forEach(i=>{const d=document.createElement("div");d.className="p-4 border border-outline-variant/20 rounded-xl bg-surface-container-lowest flex justify-between items-center hover:border-[var(--color-primary)] hover:shadow-sm transition-all",d.innerHTML=`
    ...
    `,n.appendChild(s)},Ie=(t,n)=>{const e=t&&t[0]?t[0]:{},a=e.parsed_data||{income:{},expenses:{}};document.getElementById("fin-income").textContent=f(e.monthly_income||0),document.getElementById("fin-expenses").textContent=f(e.monthly_expenses||0);const s=document.getElementById("credit-check-content"),r=document.getElementById("credit-date"),o=document.getElementById("btn-download-xml");if(!s)return;let i=document.getElementById("affordability-breakdown-list");if(!i){const c=document.querySelector("#financial-tab .grid"),p=document.createElement("div");p.id="affordability-breakdown-list",p.className="mt-6 p-6 bg-surface-container rounded-2xl border border-outline-variant/10",c.after(p),i=p}i.innerHTML=`
  ...
  `;const d=n&&n.length>0?n[0]:null;if(d){const c=d.credit_score||0,p=c>600?"text-green-600":c>500?"text-yellow-600":"text-red-600";if(r&&(r.textContent=`Checked on ${g(d.checked_at||d.created_at||new Date)}`),o){const m=d.raw_xml_data;m?(o.classList.remove("hidden"),o.innerHTML='<span class="material-symbols-outlined text-[16px] mr-1">picture_as_pdf</span> View Bureau Report',o.className="text-sm text-white px-4 py-2 rounded-xl transition-colors shadow-sm font-semibold flex items-center gap-1",o.style.background="var(--color-primary)",o.onclick=()=>window.viewBureauReport(m)):o.classList.add("hidden")}s.innerHTML=`
        <div class="p-6 border-b border-outline-variant/10 text-center bg-surface-container-lowest">
            ...
            <span class="inline-block mt-2 px-3 py-1 rounded-full bg-surface-container text-outline text-xs font-semibold border border-outline-variant/20">${d.score_band||"Standard"}</span>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-6 bg-surface-container">
            <div class="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/10 text-center">
                <span class="block text-2xl font-bold text-on-surface">${d.total_accounts||0}</span>
            ...
            <div class="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/10 text-center">
                <span class="block text-2xl font-bold" style="color:var(--color-primary)">${d.total_enquiries||0}</span>
                <span class="text-[10px] text-outline font-semibold uppercase tracking-widest mt-1">Enquiries</span>
            </div>
            <div class="bg-surface-container-lowest p-4 rounded-xl border border-outline-variant/10 text-center">
                <span class="block text-2xl font-bold text-on-surface">${d.total_judgments||0}</span>
        ...
        <div class="p-6 bg-surface-container-lowest border-t border-outline-variant/10 space-y-3">
            <div class="flex justify-between items-center border-b border-outline-variant/10 pb-2">
                ...
                <span class="font-bold text-on-surface">${f(d.total_balance||0)}</span>
            </div>
            <div class="flex justify-between items-center border-b border-outline-variant/10 pb-2">
                <span class="text-sm text-outline">Judgment Value</span>
                <span class="font-bold text-red-600">${f(d.total_judgment_amount||0)}</span>
            </div>
            ${d.ncr_reference?`
            <div class="flex justify-between items-center border-b border-outline-variant/10 pb-2">
                ...
                <span class="font-mono text-xs font-bold text-green-700 bg-green-50 px-2 py-1 rounded-lg">${d.ncr_reference}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-sm text-outline">Reported to NCR</span>
                <span class="font-bold ${d.reported_to_ncr?"text-green-600":"text-gray-400"}">
                    ${d.reported_to_ncr?"✓ Yes — "+(d.reported_at?g(d.reported_at):"Confirmed"):"Pending"}
                </span>
            </div>`:`
            <div class="flex justify-between items-center">
                <span class="text-sm text-outline flex items-center gap-1.5">
      ...
      `}else r&&(r.textContent=""),o&&o.classList.add("hidden"),s.innerHTML='<div class="py-12 text-center text-gray-400"><p>No bureau data available.</p></div>'},Be=(t,n,e)=>{const a=document.getElementById("documents-list"),s=document.getElementById("doc-count");if(!a||!s)return;const r=[{key:"idcard",label:"ID Document"},{key:"till_slip",label:"Latest Payslip"},{key:"bank_statement",label:"Bank Statement"},{key:"credit_life_contract",label:"Credit Life Contract"}];let o=0;if(e&&(e.id_front_image_url&&o++,e.id_back_image_url&&o++,e.selfie_image_url&&o++),s.textContent=(t?.length||0)+(n?1:0)+o,a.innerHTML="",r.forEach(i=>{const d=t.find(y=>y.file_type===i.key),c=i.key==="idcard"&&(e?.id_front_image_url||e?.id_back_image_url),p=d||c,m=p?"text-green-600 bg-green-100":"text-gray-400 bg-gray-100",x=p?"fa-check-circle":"fa-upload",L=c?"Verified via KYC Session":d?"File Verified":"Missing Document",w=document.createElement("div");w.className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-orange-300 transition-all group",w.innerHTML=`
        <div class="flex items-center gap-4">
      ...
      `,a.appendChild(w)}),n){const i=n.verified===!0,d=n.normalized_status||n.status||"Linked",c=document.createElement("div");c.className="flex items-center justify-between p-4 bg-blue-50/50 border border-blue-200 rounded-xl hover:border-blue-400 transition-all mt-4",c.innerHTML=`
        <div class="flex items-center gap-4">
    ...
    `,a.appendChild(c)}e&&[{key:"id_front",label:"KYC ID Front",url:e.id_front_image_url},{key:"id_back",label:"KYC ID Back",url:e.id_back_image_url},{key:"selfie",label:"KYC Selfie",url:e.selfie_image_url}].filter(d=>d.url).forEach(d=>{const c=document.createElement("div");c.className="flex items-center justify-between p-4 bg-purple-50/50 border border-purple-200 rounded-xl hover:border-purple-400 transition-all mt-4",c.innerHTML=`
          <div class="flex items-center gap-4">
      ...
      `,a.appendChild(c)}),Ae()},Ae=()=>{document.querySelectorAll(".admin-doc-upload").forEach(t=>{t.addEventListener("change",async n=>{const e=n.target.files[0];if(!e||!l)return;const a=n.target.dataset.type,s=n.target.parentElement,r=s.childNodes[0].textContent;s.childNodes[0].textContent="Processing...";try{const{data:{session:o}}=await b.auth.getSession(),i=o.user.id,d=e.name.split(".").pop(),c=`${a}_${Date.now()}.${d}`,p=`${i}/${l.user_id}_${c}`,{error:m}=await b.storage.from("client_docs").upload(p,e,{upsert:!0});if(m)throw m;const{error:x}=await b.rpc("register_admin_upload",{p_user_id:l.user_id,p_app_id:l.id,p_file_name:c,p_original_name:e.name,p_file_path:p,p_file_type:a,p_mime_type:e.type,p_file_size:e.size});if(x)throw x;u("Document Updated Successfully","success"),h()}catch(o){console.error(o),u(o.message,"error")}finally{s.childNodes[0].textContent=r}})})};window.handleSmartDownload=async t=>{try{let n=t;t.includes("/storage/v1/object/")&&(n=t.split("/").slice(8).join("/"));let{data:e,error:a}=await b.storage.from("client_docs").createSignedUrl(n,60);if((a||!e)&&({data:e,error:a}=await b.storage.from("documents").createSignedUrl(n,60)),a)throw a;window.open(e.signedUrl,"_blank")}catch(n){console.error("Smart Download Error:",n),u("File not found in any bucket. Please check storage manually.","error")}};const Te=async(t,n,e)=>{const a=document.getElementById("loan-history-list"),s=document.getElementById("app-history-list");let r=document.getElementById("admin-metadata-container");if(e){const o=document.getElementById("loan-tab");if(!r){r=document.createElement("div"),r.id="admin-metadata-container",r.className="mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-gray-100 pt-8";const i=Array.from(o.querySelectorAll("h3")).find(d=>d.textContent.includes("Client History"));i?o.insertBefore(r,i):o.appendChild(r)}try{const i=[e.created_by_admin,e.reviewed_by_admin].filter(Boolean),{data:d}=await b.from("profiles").select("id, full_name").in("id",i),c=d?.find(m=>m.id===e.created_by_admin)?.full_name||"System / User",p=d?.find(m=>m.id===e.reviewed_by_admin)?.full_name||"Pending Review";r.innerHTML=`
          ...
          `}catch(i){console.error("Admin UUID Lookup Error:",i)}}a&&(a.innerHTML="",t&&t.length>0?t.forEach(o=>{const i=document.createElement("div");i.className="p-3 border-b border-gray-100 last:border-0",i.innerHTML=`
            ...
            `,s.appendChild(i)}):s.innerHTML='<p class="text-sm text-gray-400 italic p-2">No other applications on record.</p>')},Re=t=>{const n=document.getElementById("credit-life-status-badge"),e=document.getElementById("credit-life-contract-summary"),a=document.getElementById("credit-life-signature-gallery"),s=document.getElementById("credit-life-view-contract-btn"),r=document.getElementById("credit-life-download-contract-btn");if(!n||!e||!a)return;const o=t?.offer_details||{},i=!!(t?.has_credit_life_insurance||o.credit_life_enabled),d=!!(o.credit_life_contract_signed&&o.credit_life_signature_data),c=o.credit_life_signed_at?g(o.credit_life_signed_at):"Not signed",p=o.credit_life_contract_version||"v1",m=o.credit_life_contract_text||"No signed contract snapshot stored.",x=o.credit_life_contract_file_path||null,L=Number(o.credit_life_total??t?.offer_credit_life_total??0);n.textContent=i?d?"Selected and signed":"Selected, signature missing":"Not selected",n.className=`px-3 py-1 text-xs font-bold rounded-full ${i?d?"bg-green-100 text-green-700":"bg-yellow-100 text-yellow-700":"bg-gray-200 text-gray-700"}`,e.innerHTML=`
  ...
  `;const w=o.signature_data,y=o.credit_life_signature_data,_=(v,D,E)=>`
    <div class="rounded-xl border border-gray-200 bg-gray-50 p-3">
      <p class="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-2">${v}</p>
      <p class="text-xs text-gray-500 mb-3">${E}</p>
      ${D?`<img src="${D}" alt="${v}" class="w-full h-40 object-contain rounded-lg border border-gray-200 bg-white">`:'<div class="h-40 rounded-lg border border-dashed border-gray-300 bg-white flex items-center justify-center text-sm text-gray-400">No signature captured</div>'}
    </div>
  `;a.innerHTML=[_("Main Loan Signature",w,"Captured from the standard loan acknowledgement step."),_("Credit Life Signature",y,"Captured only when the Credit Life contract is signed.")].join(""),s&&(s.classList.toggle("hidden",!i),s.onclick=()=>Ne(t)),r&&(r.classList.toggle("hidden",!x),r.onclick=()=>{x&&(/^https?:\/\//i.test(x)?window.open(x,"_blank"):handleSmartDownload(x))})},Ne=t=>{const n=document.getElementById("credit-life-contract-modal"),e=document.getElementById("credit-life-contract-modal-body");if(!n||!e)return;const a=t?.offer_details||{},s=a.credit_life_signed_at?g(a.credit_life_signed_at):"Not signed",r=a.credit_life_contract_version||"v1",o=a.credit_life_contract_text||"No signed contract snapshot stored.",i=a.signature_data,d=a.credit_life_signature_data,c=(p,m)=>`
  ...
  `;e.innerHTML=`
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      ${c("Credit Life Signature",d)}
    </div>
  `,n.classList.remove("hidden"),n.classList.add("flex")},Y=()=>{const t=document.getElementById("credit-life-contract-modal");t&&(t.classList.add("hidden"),t.classList.remove("flex"))},j=t=>{if(!t)return;const n=t.status||"pending",e=document.getElementById("sidebar-status"),a=document.getElementById("status-alert"),s=document.getElementById("action-buttons-container"),r=parseFloat(t.offer_principal||t.amount||0),o=parseInt(t.term_months||1),i=parseFloat(t.offer_total_interest||0),d=parseFloat(t.offer_total_initiation_fees||0),c=parseFloat(t.offer_total_admin_fees||0),p=parseFloat(t.offer_details?.credit_life_total||t.offer_credit_life_total||0),m=parseFloat(t.offer_total_repayment||0),x=parseFloat(t.offer_monthly_repayment||0),L=parseFloat(t.offer_interest_rate||0),w=t.repayment_start_date||t.offer_details?.first_payment_date;document.getElementById("sidebar-amount").textContent=f(r),document.getElementById("sidebar-term").textContent=`${o} Month${o>1?"s":""}`,document.getElementById("sidebar-payment").textContent=f(x);let y=document.getElementById("financial-breakdown");if(!y){const E=document.getElementById("sidebar-payment").parentElement.parentElement;y=document.createElement("div"),y.id="financial-breakdown",y.className="pt-4 border-t border-gray-100 space-y-4",E.after(y)}y.innerHTML=`
    <div class="space-y-3 bg-gray-50 p-4 rounded-xl border border-gray-200">
        ...
        <div class="flex justify-between items-center text-xs">
            ...
            <span class="font-bold text-gray-900">${f(d)}</span>
        </div>
        <div class="flex justify-between items-center text-xs">
  ...
  `,e&&(e.textContent=n.replace("_"," "),e.className=`mt-2 text-lg font-bold uppercase tracking-wide ${U(n).split(" ")[0].replace("bg-","text-").replace("-100","-600")}`);const _=document.getElementById("status-override-select"),v=document.getElementById("manual-update-btn"),D=document.getElementById("override-hint");if(n==="DISBURSED"?(_&&(_.disabled=!0),v&&(v.disabled=!0,v.classList.add("opacity-50","cursor-not-allowed"),v.innerText="Locked"),D&&(D.textContent="🔒 Application is active. Modifications disabled.")):(_&&(_.disabled=!1,_.value=n),v&&(v.disabled=!1,v.innerText="Update")),a&&(a.className="mt-3 p-3 rounded-lg text-xs font-medium leading-relaxed hidden",n==="OFFERED"?(a.textContent="Contract Sent. Waiting for user to sign.",a.classList.add("bg-purple-50","text-purple-700","block")):n==="READY_TO_DISBURSE"&&(a.textContent="Application is queued for disbursement.",a.classList.add("bg-green-50","text-green-700","block"))),s){if(s.innerHTML="",["BUREAU_OK","BANK_LINKING","STARTED","AFFORD_REFER","BUREAU_REFER"].includes(n)){const E=n==="AFFORD_REFER"||n==="BUREAU_REFER"?'<div class="p-3 bg-orange-50 border border-orange-100 rounded-lg mb-3 text-xs text-orange-700 font-bold"><i class="fa-solid fa-circle-exclamation mr-1"></i> Currently Under Manual Review</div>':"";s.innerHTML=`
          ...
          `,document.getElementById("action-send-contract")?.addEventListener("click",E=>xe(E.currentTarget)),document.getElementById("action-preview-contract")?.addEventListener("click",ge);else if(n==="OFFER_ACCEPTED")s.innerHTML=`
          ...
          `,document.getElementById("btn-approve-contract").onclick=()=>Ce("Approve","Mark contract as valid and ready for payout?",$e);else if(n==="READY_TO_DISBURSE")s.innerHTML='<div class="p-4 bg-green-50 border border-green-100 rounded-xl text-center"><p class="text-sm font-bold text-green-800">Queued for Payout</p></div>';else if(n==="DISBURSED")s.innerHTML='<div class="p-4 bg-gray-50 border border-gray-100 rounded-xl text-center"><p class="text-sm font-bold text-gray-600">Loan Active / Completed</p></div>';else if(n==="IN_ARREARS")s.innerHTML=`
            ...
            </button>`;else if(n==="IN_DEFAULT"){const G=(parseFloat(t.offer_principal||t.amount||0)*.03).toLocaleString("en-ZA",{minimumFractionDigits:2});s.innerHTML=`
            <div class="p-3 bg-red-50 border border-red-200 rounded-xl mb-3 text-xs text-red-800 font-bold">
              ...
              <div class="font-normal mt-1 text-red-700">Default interest: R ${G}</div>
            </div>
            <button onclick="window.open('/api/letters-of-demand/${t.id}', '_blank')"
            ...
            </button>`}}},Fe=t=>{if(!t)return;document.getElementById("applicant-name-header").textContent=t.profiles?.full_name||"Unknown";const n=t.profiles||{},e=n.client_number?String(n.client_number):"",a=t.loan_number?`L${String(t.loan_number).padStart(4,"0")}`:"",s=e&&a?`${e}-${a}`:a||t.id.slice(0,8).toUpperCase(),r=t.agreement_number||s;document.getElementById("header-id-val").textContent=s,document.getElementById("header-date").textContent=g(t.created_at),document.getElementById("detail-app-id").textContent=r,document.getElementById("detail-date").textContent=g(t.created_at),document.getElementById("detail-purpose").textContent=t.loan_purpose||t.purpose||"Personal Loan",document.getElementById("detail-notes").value=t.notes||"";const o=document.getElementById("header-status-badge");o&&(o.textContent=t.status,o.className=`px-4 py-1.5 text-sm font-bold rounded-full shadow-sm ${U(t.status)}`)};window.saveEmployerDetails=async function(){if(!l?.user_id)return;const t=document.getElementById("employer-name-input")?.value.trim(),n=document.getElementById("employer-phone-input")?.value.trim(),e=document.getElementById("employer-address-input")?.value.trim(),a=document.getElementById("btn-save-employer");a.textContent="Saving…",a.disabled=!0;try{const{error:s}=await b.from("profiles").update({employer_name:t||null,employer_phone:n||null,employer_address:e||null}).eq("id",l.user_id);if(s)throw s;u("Employer details saved.","success")}catch(s){u(s.message,"error")}finally{a.textContent="Save Details",a.disabled=!1}};window.toggleEmployerVerified=async function(){if(!l?.user_id)return;const t=document.getElementById("employer-verified-badge"),n=document.getElementById("employer-verified-note"),e=document.getElementById("btn-verify-employer"),s=!(t?.textContent==="Verified"),{data:{session:r}}=await b.auth.getSession(),{data:o}=r?await b.from("profiles").select("full_name").eq("id",r.user.id).maybeSingle():{data:null},{error:i}=await b.from("profiles").update({employer_verified:s,employer_verified_at:s?new Date().toISOString():null,employer_verified_by:s?o?.full_name||r?.user?.email||"Admin":null}).eq("id",l.user_id);if(i){u(i.message,"error");return}t&&(t.textContent=s?"Verified":"Unverified",t.className=`px-2 py-1 rounded-full text-xs font-bold ${s?"bg-green-100 text-green-700":"bg-gray-100 text-gray-500"}`),e&&(e.textContent=s?"Revoke Verification":"Mark Verified"),n&&(n.textContent=s?`Verified by ${o?.full_name||"Admin"} on ${new Date().toLocaleDateString("en-ZA")}`:"",n.classList.toggle("hidden",!s)),u(s?"Employer verified.":"Verification revoked.","success")};function je(t){const n=document.getElementById("employer-name-input"),e=document.getElementById("employer-phone-input"),a=document.getElementById("employer-address-input"),s=document.getElementById("employer-verified-badge"),r=document.getElementById("employer-verified-note"),o=document.getElementById("btn-verify-employer");n&&(n.value=t?.employer_name||""),e&&(e.value=t?.employer_phone||""),a&&(a.value=t?.employer_address||"");const i=t?.employer_verified===!0;s&&(s.textContent=i?"Verified":"Unverified",s.className=`px-2 py-1 rounded-full text-xs font-bold ${i?"bg-green-100 text-green-700":"bg-gray-100 text-gray-500"}`),o&&(o.textContent=i?"Revoke Verification":"Mark Verified"),r&&i&&t.employer_verified_at&&(r.textContent=`Verified by ${t.employer_verified_by||"Admin"} on ${new Date(t.employer_verified_at).toLocaleDateString("en-ZA")}`,r.classList.remove("hidden"))}window.saveClientCap=async function(){if(!l?.user_id)return;const t=document.getElementById("credit-cap-input")?.value,n=document.getElementById("credit-cap-note")?.value.trim(),e=t?parseFloat(t):null,{error:a}=await b.from("profiles").update({credit_limit_override:e,credit_limit_note:n||null}).eq("id",l.user_id);if(a){u(a.message,"error");return}const s=document.getElementById("credit-cap-current");s&&(s.textContent=e?`Current cap: R${e.toLocaleString("en-ZA")} — ${n||""}`:"No cap set — using standard band rules."),u(e?`Credit cap set to R${e.toLocaleString("en-ZA")}.`:"Credit cap removed.","success"),await b.from("audit_log").insert([{entity_type:"profile",entity_id:l.user_id,action:"credit_cap_set",new_value:{cap:e,note:n},description:e?`Credit cap set to R${e.toLocaleString("en-ZA")}`:"Credit cap removed"}]).catch(()=>{})};function Me(t){const n=document.getElementById("credit-cap-input"),e=document.getElementById("credit-cap-note"),a=document.getElementById("credit-cap-current");n&&t?.credit_limit_override&&(n.value=t.credit_limit_override),e&&t?.credit_limit_note&&(e.value=t.credit_limit_note),a&&(a.textContent=t?.credit_limit_override?`Current cap: R${Number(t.credit_limit_override).toLocaleString("en-ZA")}${t.credit_limit_note?" — "+t.credit_limit_note:""}`:"No cap set — using standard band rules.")}let T=[];async function J(t){try{T=(await(await fetch(`/api/audit-log/loan_application/${t}`)).json()).data||[],Oe()}catch(n){console.warn("[audit-trail]",n.message)}}function Oe(){const t=document.getElementById("audit-trail-list");if(!t)return;if(!T.length){t.innerHTML='<div class="text-center py-8 text-sm text-gray-400">No audit entries yet. Changes will appear here.</div>';return}const n={status_change:{icon:"swap_horiz",color:"#3b82f6"},field_update:{icon:"edit",color:"#f59e0b"},created:{icon:"add_circle",color:"#10b981"},viewed:{icon:"visibility",color:"#8b5cf6"},default:{icon:"history",color:"#6b7280"}};t.innerHTML=T.map(e=>{const a=n[e.action]||n.default,s=new Date(e.created_at),r=s.toLocaleDateString("en-ZA",{day:"numeric",month:"short",year:"numeric"}),o=s.toLocaleTimeString("en-ZA",{hour:"2-digit",minute:"2-digit"});let i="";return e.action==="status_change"&&e.old_value?.status&&e.new_value?.status?i=`<span class="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded">${e.old_value.status}</span>
...
`),a=new Blob([e],{type:"text/csv;charset=utf-8;"}),s=URL.createObjectURL(a),r=document.createElement("a");r.href=s,r.download=`audit_trail_${new Date().toISOString().slice(0,10)}.csv`,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(s)};const h=async()=>{const n=new URLSearchParams(window.location.search).get("id");if(n)try{const e=await X(n);l=e,M(),document.getElementById("contract-declined-banner")?.remove(),Fe(e),Le(e.profiles||{},e.bank_accounts),await De(e.user_id),Ie(e.financial_profiles,e.credit_checks),Be(e.documents,e.truid_info,e.kyc_info),await Te(e.loan_history,e.application_history,e),Re(e),J(n),j(e),ke(e),await fe(),document.getElementById("loading-state")?.classList.add("hidden"),document.getElementById("content-grid")?.classList.remove("hidden"),document.getElementById("page-header")?.classList.remove("hidden")}catch(e){console.error("Integration Error:",e),u("Failed to load full application data.","error")}};document.addEventListener("DOMContentLoaded",async()=>{await Q();let t=document.getElementById("main-content");t||(t=document.createElement("main"),t.id="main-content",t.className="flex-1 p-6 pt-24",document.getElementById("app-shell").appendChild(t)),t.innerHTML=be,Se(),await h(),document.getElementById("btn-save-notes")?.addEventListener("click",saveNotes);const n=document.getElementById("modal-confirm-btn"),e=document.getElementById("modal-cancel-btn");n&&n.addEventListener("click",()=>{typeof F=="function"&&F()}),e&&e.addEventListener("click",C),document.getElementById("credit-life-contract-modal-close")?.addEventListener("click",Y),document.getElementById("credit-life-contract-modal")?.addEventListener("click",a=>{a.target?.id==="credit-life-contract-modal"&&Y()})});
